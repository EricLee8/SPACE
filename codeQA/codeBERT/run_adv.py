...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    if os.path.exists(cache_dir):
        ...
        return features
    if args.entity_adv and stage=='train':
    ...
    for example_index, example in tqdm(enumerate(examples), total=len(examples)):
        ...
        if args.entity_adv and stage=='train':
            ori_code = example.source_code
            tree = parser[0].parse(bytes(example.source_code, 'utf8'))
            ...
            variable_tokens = tree_to_variable_mask(root_node, index_to_code, ori_code)
            adversarial_masks = [1 if x in variable_tokens else 0 for x in code_tokens]
            source_code_tokens = [tokenizer.tokenize('@ '+x)[1:] if idx!=0 else tokenizer.tokenize(x) for idx,x in enumerate(code_tokens)]
            source_code_tokens = [y for x in source_code_tokens for y in x]
            adv_masks = [[adversarial_masks[idx]] * (len(tokenizer.tokenize('@ '+x)[1:]) if idx!=0\
                     else len(tokenizer.tokenize(x))) for idx, x in enumerate(code_tokens)]
            adv_masks = [y for x in adv_masks for y in x]
            adv_masks = [0]*len(source_question_tokens) + [0] + adv_masks
        ...
        if args.entity_adv and stage=='train':
            adv_masks = adv_masks[:args.max_source_length-2]
            adv_masks = [0] + adv_masks + [0]
            adv_masks += [0]*padding_length
        else:
            adv_masks = None
        #target
...
def main():
    ...
    if args.do_train:
        ...
        if args.entity_adv:
            all_adv_masks = torch.tensor([f.adv_masks for f in train_features], dtype=torch.long)
            train_data = TensorDataset(all_source_ids,all_source_mask,all_target_ids,all_target_mask,all_adv_masks)
        else:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for batch in bar:
                ...
                if args.entity_adv:
                    source_ids,source_mask,target_ids,target_mask,adv_masks = batch
                else:
                    source_ids,source_mask,target_ids,target_mask = batch

                embeds_init=model.encoder.embeddings.word_embeddings(source_ids) # (bsz, slen, hsz)
                ...
                for astep in range(args.adv_steps):
                    ...
                    if args.fp16:
                        with autocast():
                            loss,_,_ = model(source_ids=source_ids,source_mask=source_mask,target_ids=target_ids,target_mask=target_mask,delta=delta)
                            if args.n_gpu > 1:
                    ...
                    embeds_init = model.encoder.embeddings.word_embeddings(source_ids)
