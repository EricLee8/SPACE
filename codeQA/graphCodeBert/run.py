...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    if not os.path.exists("caches"):
        os.mkdir("caches")
    cache_dir = os.path.join("caches", '{}_{}_{}.features'.format(args.lang, stage, args.max_source_length))
    ...
    for example_index, example in tqdm(enumerate(examples), total=len(examples)):
        ...
        if example_index < 5:
            if stage=='train':
                ...
                logger.info("adv_masks: {}".format(None))
...
class TextDataset(Dataset):
    def __init__(self, features, tokenizer, args, training=True):
...
def main():
    ...
    if args.do_train:
        ...
        train_data = TextDataset(train_features, tokenizer, args, training=True)
