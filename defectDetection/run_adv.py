...
def extract_dataflow_entity_adv(code, parser, lang):
    ...
    #obtain dataflow 
    ori_code=code
    if len(ori_code.split(' ')) > 2000:
        code = ' '.join(ori_code.split(' ')[:2000])
        ori_code = code
    tree = parser[0].parse(bytes(code,'utf8'))
    root_node = tree.root_node
    tokens_index=tree_to_token_index(root_node)
...
def convert_examples_to_features(item):
    ...
    #extract data flow
    if args.entity_adv:
        ori_code_tokens,dfg,variable_poses=extract_dataflow_entity_adv(js['func'],parser,'c')
    else:
        ori_code_tokens,dfg=extract_dataflow(js['func'],parser,'c')
    ...
    #truncating
    dfg_len = len(dfg)
    code_tokens=code_tokens[:args.code_length+args.data_flow_length-3-min(dfg_len,args.data_flow_length)][:512-3]
    ...
    if args.entity_adv:
        ...
        var_poses=var_poses[:args.code_length+args.data_flow_length-3-min(dfg_len,args.data_flow_length)][:512-3]
    ...
    return InputFeatures(source_tokens,source_ids,position_idx,\
        dfg_to_code,dfg_to_dfg,js['commit_id'],js['target'],var_poses)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None,pool=None):
        ...
        cache_file=args.output_dir+'/'+prefix+'_{}_{}{}'.format(args.code_length,\
             args.data_flow_length, '_advpos' if args.entity_adv else '')+'.pkl'
...
def train(args, train_dataset, model, tokenizer, pool):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            if args.entity_adv:
                (inputs_ids,position_idx,attn_mask,labels,var_poses)=[x.to(args.device) for x in batch]
            else:
            ...
            if isinstance(model, torch.nn.DataParallel):
                ...
                embeds_init=model.encoder.roberta.embeddings.word_embeddings(inputs_ids)
            ...
            for astep in range(args.adv_steps):
                ...
                if args.fp16:
                    with autocast():
                        loss, _ = model(inputs_ids,position_idx,attn_mask,labels,delta=delta)
                        if args.n_gpu > 1:
                            loss = loss.mean()
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_acc']>best_acc:
                        ...
                        checkpoint_prefix = '{}_{}_{}{}'.format(args.save_name,\
                             args.code_length, args.data_flow_length, '_advpos' if args.entity_adv else '')
...
def test(args, model, tokenizer, best_threshold=0, pool=None, adv=False):
    #build dataloader
...
def main():
    ...
    results = {}
    if args.do_eval:
        checkpoint_prefix = '{}_{}_{}{}/model.bin'.format(args.save_name,\
             args.code_length, args.data_flow_length, '_enadv' if args.entity_adv else '')
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
